    ...
    <div className="min-h-screen py-12 px-4">
      <div className="container mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-12">
          <h1 className="mb-2" style={{ fontSize: '3rem', fontWeight: 600 }}>{collectionTitle}</h1>
          <p className="text-muted-foreground">{filteredProducts.length} منتجات</p>
        </motion.div>

        <div className="flex flex-col md:flex-row gap-8">
          <motion.aside initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="md:w-64 space-y-6">
            <div className="bg-card p-6 rounded-sm border border-border">
              ...
              <div className="space-y-6">
                ...
                {(selectedCategory !== 'all' || priceRange !== 'all') && (
                  <Button variant="outline" size="sm" onClick={() => { setSelectedCategory('all'); setPriceRange('all'); }} className="w-full">
                    مسح الفلاتر
                  </Button>
                )}
              </div>
                  ...
                  <motion.div key={product.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
                    <Link to={`/product/${product.id}`} className="group">
                      ...
                      <div className="space-y-1">
                        <h3 className="uppercase tracking-wider text-sm group-hover:text-accent transition-colors">{product.name}</h3>
