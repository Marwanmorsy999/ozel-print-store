  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const orderPayload = {
        ...
        items, // includes custom design data URLs so the design is saved with the order
        total_price: total,
        currency: 'EGP',
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-12">
        ...
        >
          ...
          <p className="font-display text-lg font-bold mb-5">#{placedOrder.id}</p>
                ...
                <div className="space-y-4 max-h-72 overflow-y-auto pr-1">
                  {items.map(item => (
                    <div key={item.key} className="flex gap-3">
                      ...
                      <span className="text-sm font-display font-semibold whitespace-nowrap">{item.price * item.quantity} ج</span>
                    </div>
                  ))}
                </div>
